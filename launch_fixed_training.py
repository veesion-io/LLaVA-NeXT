...
def request_training():
    """Request a new training job with fixed video description generation and memory logging"""
    
    # Generate training name with timestamp
    timestamp = datetime.now().strftime("%Y-%m-%d-%H-%M-%S")
    training_name = f"llava_cheater_fixed-{timestamp}"
    
    print(f"🚀 Launching fixed training: {training_name}")
    
    # Training configuration with fixed callbacks and 1.5x batch size
    cmd = [
        ...
        "--description", f"Fixed video training with proper memory logging and video descriptions - {training_name}"
    ]
