...
def main():
    ...
    if workflow_run:
        ...
        if instance_ip:
            print(f"\n🚀 Training '{args.training_name}' is running on instance: {instance_ip}")
            # Here you can add logic to SSH or monitor the instance
        else:
            print(f"\n❌ Could not get instance IP for training '{args.training_name}'")
    else:
        print(f"\n❌ Could not find a successful workflow for training '{args.training_name}'")
